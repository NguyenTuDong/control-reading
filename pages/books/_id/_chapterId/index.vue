<template>
  <main class="p-chapter">
    ...
    <div class="c-control">
      <button class="c-control__button" @click="togglePanel">
        <i class="fas fa-cog"></i>
      </button>
      <div class="c-control__panel">
        <label class="c-switch" for="gesture_switch"
          >Gesture reading:
          <input v-model="readingGesture" type="checkbox" id="gesture_switch" />
          <span class="c-switch__slider"></span>
        </label>
        <label class="c-slider" for="scroll_speed"
          >Scroll speed: <span class="u-primary-color">{{ scrollSpeed }}</span>
          <input
            v-model="scrollSpeed"
            type="range"
            min="20"
      ...
      <div class="c-control__loading" :class="{ show: loading }">
        Loading gesture...
      </div>
    </div>
  </main>
...
export default {
  ...
  computed: {
    ...
    webcam: {
      get() {
        return this.$store.state.webcam
      },
      set(value) {
        let self = this
        self.$store.commit('setWebcam', value)
      },
    },
  },
  mounted() {
  ...
  async beforeDestroy() {
    this.isActive = false
    if (this.$route.name != 'books-id-chapterId') {
      if (this.webcam) {
  ...
  methods: {
    ...
    async predict() {
      ...
      const { pose, posenetOutput } = await self.model.estimatePose(
        self.webcam.canvas
      )
